...
**如果你只看一个 OpenClaw 仓库，先看这个：把读书笔记、人物资料和商业事件做成可搜索、可筛选、可发布的静态知识库。**

> 这个仓库建议作为 OpenClaw 第一波主推入口：它最容易被读书、知识管理、前端和内容产品用户理解。
...
Fork 后替换 `data.js` 和 `books.js`，就能做成自己的读书卡片库、人物库或行业资料库。

你主要需要替换：`data.js` / `books.js`。
